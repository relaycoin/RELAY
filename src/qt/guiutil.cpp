...
void setupAddressWidget(QValidatedLineEdit* widget, QWidget* parent)
{
    ...
    widget->setPlaceholderText(QObject::tr("Enter a REL address (e.g. %1)").arg("RCiv11J94BnvLhj1MenP6By51YAx4xjXT2"));
#endif
...
{
    // return if URI is not valid or is no REL: URI
...
bool parseBitcoinURI(QString uri, SendCoinsRecipient* out)
{
    // Convert rel:// to rel:
    //
    //    Cannot handle this later, because rel:// will cause Qt to see the part after // as host,
...
void openConfigfile()
{
    ...
    /* Open rel.conf with the associated application */
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    if (!fAutoStart)
        ...
        if (!optionFile.good())
            return false;
        // Write a rel.desktop file to the autostart directory:
...
LSSharedFileListItemRef findStartupItemInList(LSSharedFileListRef list, CFURLRef findUrl)
{
    // loop through the list of startup items and try to find the rel app
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    ...
    if (fAutoStart && !foundItem) {
        // add rel app to startup item list
